...
export class ListusuarioComponent implements OnInit {
  ...
  currentUrl: string = `${this.userService.BASE_URL}/api/usuarios/`; // o usa una constante/baseURL
  count: number = 0;
  ...
  ngOnInit(): void {
    this.loadUsuarios(this.currentUrl);
  }

  async loadUsuarios(url: string) {
    try {
      const token: any = localStorage.getItem('authToken');
      const response = await this.userService.allUserPaginated(url, token);
  ...
  async deleteUser(userId: string) {
    ...
    if (confirmDelete) {
      try {
        ...
        this.loadUsuarios(this.currentUrl); // recargar la página actual
      } catch (error: any) {
